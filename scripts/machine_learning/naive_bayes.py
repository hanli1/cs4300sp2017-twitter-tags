...
if __name__ == "__main__":
    ...
    if train_or_predict == "train":
        ...
        # Build bag of words vector for each tweet based on the dictionary of words used to train the Naive Bayes model
        ...
        for user in all_tokenized_tweets_dict:
            ...
            bag_of_words_vectors_dict[user] = current_bag_of_words_vectors
        # Predict each tweet's label using Naive Bayes
        with open("trained_naive_bayes_model/trained_model", 'r') as f:
            nb_model = pickle.load(f)
        tags = nb_model.classes_
        # Contains mappings from a user to the tags determined for their tweets (a tag is associated with a user if a
        # certain proportion of their tweets fall under that tag)
        user_tags_dict = {}
        for user in bag_of_words_vectors_dict:
            user_tag_label_counts = [0] * len(tags)
            prediction_probs = nb_model.predict_proba(bag_of_words_vectors_dict[user])
            ...
            for i in range(len(tags)):
                if (float(user_tag_label_counts[i]) / sum(user_tag_label_counts)) > 0.2:
